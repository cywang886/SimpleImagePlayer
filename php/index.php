...
<html>
	...
	<body>
		...
		<div class="imageDiv" id="imageDiv">
			<img id="imageContent" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAJcEhZcwAAEnQAABJ0Ad5mH3gAAAAMSURBVBhXY/j//z8ABf4C/qc1gYQAAAAASUVORK5CYII=" style="display:none;" />
		</div>
		...
		// 占位图片,必须为base64,而且必须跟imageContent.src一致
		var imageHolder = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAJcEhZcwAAEnQAABJ0Ad5mH3gAAAAMSURBVBhXY/j//z8ABf4C/qc1gYQAAAAASUVORK5CYII=';
		</script>
		...
		<script>
			...
			var changeImage = function(imageUrl) {
					if (imageUrl == "") {
						console.error('Now Allow Image Change to Null');
						return false;
					}
					console.log('Image Change to ' + imageUrl);
					$("#imageContent").attr('src', imageUrl);
					if (imageUrl != errorImgUrl && imageUrl != imageHolder) {
					...
					$.ajax({
						...
						success: function(ajaxResult) {
							if (ajaxResult.success) {
								if (ajaxResult.image_url == "") {
									console.error('Refresh Image Failed (Server Error): 服务器返回的数据异常,操作成功但没有图片数据');
									changeImage(errorImgUrl);
									layer.ready(function() {
										layer.alert('服务器返回的数据异常,操作成功但没有图片数据', {
											title: '错误'
										});
									});
									// layer.closeAll('loading');
								} else {
									changeImage(ajaxResult.image_url);
								}
							} else {
								console.error('Refresh Image Failed (Server Error): ', ajaxResult.message);
								changeImage(errorImgUrl);
								layer.ready(function() {
									layer.alert(ajaxResult.message, {
										title: '错误'
									});
								});
								// layer.closeAll('loading');
			...
			$('#imageContent').on('error', function(){
				// 对内部地址加载失败不做任何处理
				if ($("#imageContent")[0].src == "" || $("#imageContent")[0].src == imageHolder) {
					return false;
				}
				layer.ready(function() {
					layer.closeAll('loading');
				...
				if ($("#imageContent")[0].src == errorImgUrl) {
					// 加载失败提示图片也加载失败了,停止套娃
					layer.ready(function() {
						layer.alert('异常错误,加载失败友好提示图片加载失败', {
							title: '错误'
						});
					});
					console.error('异常错误,加载失败友好提示图片加载失败');
					return false;
				}
				layer.ready(function() {
					layer.load();
			...
			$('#imageContent').on('load', function(){
				...
				// 对初始化时加载不做任何处理
				if ($("#imageContent")[0].src == imageHolder) {
					return false;
				}
				if ($("#imageContent").is(":hidden")) {
					// 默认隐藏,避免出现破图符号
					console.log('Img Container Displayed');
					// 取消隐藏
					$("#imageContent").attr('style', '');
				}
				layer.ready(function() {
					layer.closeAll('loading');
				});
				// 显示当前图片路径
				if (nowImageUrl != '' && $("#imageContent")[0].src != errorImgUrl) {
					console.log('Now Image Path: ' + nowImageUrl);
				}
			});
			...
			var checkTouchSupport = function(){
				if (IsMobile()) {
					try {
						...
						layer.ready(function() {
							layer.alert('非常抱歉,该移动端设备不支持Touch Event,已切换到PC模式', {
								title: '提示'
							});
						});
						return false;
